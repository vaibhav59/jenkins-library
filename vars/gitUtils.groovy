...
String getBranchName(scmvars) {
    ...
    return scmvars.GIT_BRANCH ? sh(returnStdout: true, script: """ echo  \${script.scmvars.GIT_BRANCH} """).trim() : sh(returnStdout: true, script: """git rev-parse --abbrev-ref HEAD | awk -F/ '{print \$2}' """).trim()
}
