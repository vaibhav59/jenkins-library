...
String getBranchName(scmvars) {
    ...
    return scmvars.GIT_BRANCH ? sh(returnStdout: true, script: """ echo  \${script.scmvars.GIT_BRANCH} | sed 's/origin\\///g' """).trim() : sh(returnStdout: true, script: """git rev-parse --abbrev-ref HEAD | awk -F/ '{print \$2}' """).trim()
}
